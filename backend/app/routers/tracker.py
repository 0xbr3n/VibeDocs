...
def export_cloud_tracker(
    ...
    * Info sheet              -> the AWS account IDs (Report Details field).
    ...
    # No bundled "Cloud VAPT" tracker template? (e.g. the open-source build ships
    # none.) Fall back to a synthesised single-sheet Risk Register instead of
    # 500ing — the standard tracker export already degrades this way.
    synth = (not template_path) or (not Path(template_path).exists())
    ...
    try:
        if synth:
            # Synthesised mode: one Risk Register sheet with all findings
            # (VA + PT). The dual-sheet / per-service layout needs the branded
            # template's named sheets, which a synthesised workbook lacks.
            rr_svc.write_risk_register(
                va_dicts + pt_dicts,
                template_path=None,
                output_path=out_path,
            ...
            try:
                ...
                rr_svc.write_risk_register(
                    ...
                    # Strip RMM cols on the final pass (covers VA + PT + per-service).
                    rmm_enabled=bool(details.get("rmm_enabled", True)),
                )
            finally:
                try:
